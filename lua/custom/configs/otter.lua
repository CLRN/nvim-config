...
local function action_fallback(action)
  return function()
    return utils.ft.markdown.in_codeblock() and ot["ask_" .. action]() or vim.lsp.buf[action]()
  end
end
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(info)
    vim.schedule(function()
      local buf = info.buf
      if not vim.api.nvim_buf_is_valid(buf) or not vim.bo[buf].ma or not utils.treesitter.is_active(buf) then
        return
      end
      ...
      vim.keymap.set('n', '<leader>lr', action_fallback('rename'),          { buffer = buf })
      vim.keymap.set('n', '<leaer>lh',  action_fallback('hover'),           { buffer = buf })
      vim.keymap.set('n', 'gr',         action_fallback('references'),      { buffer = buf })
      vim.keymap.set('n', 'gD',         action_fallback('type_definition'), { buffer = buf })
      vim.keymap.set('n', 'gd',         action_fallback('definition'),      { buffer = buf })
      vim.keymap.set('n', 'fm;',        action_fallback('format'),          { buffer = buf })
      -- stylua: ignore end
    end)
  end,
})
...
if cmp_ok then
  cmp.setup {
    sources = table.insert(require("cmp.config").get().sources, { name = "otter" }),
  }
end
